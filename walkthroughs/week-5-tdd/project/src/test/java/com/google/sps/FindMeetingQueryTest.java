...
public final class FindMeetingQueryTest {
  ...
  @Test
  public void optionalAttendeesNotConsideredWhenUnableTo() {
    // Have each mandatory person have different events, but one optional attendee with an all-day
    // event. We should see three options as if the optional attendee was not being considered.
    // Persons A and B are mandatory attendees, Person C is an optional attendee.
    //
    // Events  : |--------------C--------------|
    // Events  :       |--A--|     |--B--|
    // Day     : |-----------------------------|
  ...
  @Test
  public void optionalAttendeesConsideredWhenPossible() {
    // Have each mandatory person have different events, but one optional attendee with an event
    // that eliminates what would be an available time slot for the mandatory attendees alone.
    // We should see two options returned, taking into consideration the optional attendee.
    // Persons A and B are mandatory attendees, Person C is an optional attendee.
    //
    // Events  :             |--C--|
    // Events  :       |--A--|     |--B--|
  ...
  @Test
  public void justEnoughRoomIgnoresOptionalAttendeeIfClashes() {
    // Have one person, but make it so that there is just enough room at one point in the day to
    // have the meeting. Also have an optional attendee (Person B), who has an event in this free
    // time slot so that the optional attendee is ignored.
    //
    // Events  : |--A--|     |----A----|
    // Events  :       |-B-|
    // Day     : |---------------------|
  ...
  public void onlyOptionalAttendees() {
    // A meeting request with no mandatory attendees, but two optional attendees with gaps in their
    // schedules.
    // We should see options are returned that still that works for those attendees.
    // Events  :       |--A--|     |--A--|
    // Events  : |------B------|
    // Day     : |-----------------------------|
    // Options :                |--|     |-----|
  ...
  @Test
  public void onlyOptionalAttendeesWithNoFreeTime() {
    // No mandatory attendees, only two optional attendees with no gaps in their schedule.
    // We should see the option returning the whole day as available as the optional attendees can
    // not be accommodated and the mandatory ones (all zero of them) are free all day.
    // Events  : |--------------A--------------|
    // Events  : |--------------B--------------|
    // Day     : |-----------------------------|
    // Options : |-----------------------------|
